## Function for plotting means with 95% Confidence Interval for within-subjects
## comparisons, offset by off on the x-axis
## written by Dr. Prof. K. Oberauer
...
## x = number of column within iv that is used as the x-axis (by default the
## first column in iv)

lineplot.ci <- function(data, dv, iv, id=1, group=NULL, x=1, off=0, Bakeman=TRUE,
                        upper=TRUE, lower=TRUE, ylim=c(0,1), xlim=NULL, pt=15:25,
                        col=rep("black", 11), ptcol=rep("white",11),na.rm=TRUE,
                        cex=1, ...){
    ## prevent mis-match of character and numeric indices into columns
    if (is.character(iv) & is.numeric(id)) id = names(data)[id]

    ## computing Bakeman & McArthur correction (for long data): id = column
    ## with subject id, dv = column with dependent variable
    idvar <- data[,id]
    ...
    if (Bakeman) {
        if (!is.null(group)) {
            ...
            for (g in 1:length(glevels)) {
                ## assign integer indices to the group levels
        ...
        for (ii in 1:length(ids)) {
            corrdata[data[,id]==ids[ii],dv] <- corrdata[data[,id]==ids[ii],dv] -
                subjMeans[subjMeans[,id]==ids[ii],2] +
                grandMean[groupVar[groupVar$ids==ids[ii], "groupidx"]]
        }
    }
    ## first step: individual subject's mean per condition
    ...
    ## second step: average over subjects (IVs start in column 2)
    ...
    if (is.null(xlim)) xlim=c(xaxis[1]-0.5*(xaxis[2]-xaxis[1]),
                              xaxis[length(xaxis)]+0.5*(xaxis[2]-xaxis[1]))
    xaxt <- "s"
    ...
    for (j in 1:loop)  {
      ...
      CIlow <- y - dci
      xx <- xx + (j-1)*off
      plot(xx, y, xlim=xlim, ylim=ylim, type="b", pch=pt[j], col=col[j],
           bg=ptcol[j], xaxt=xaxt, cex=cex, ...)  
      Hmisc::errbar(xx, y, yplus=CIhigh, yminus=CIlow, xlim=xlim, ylim=ylim,
                    add=T,type="l", xaxt=xaxt, ...)
      ## if x-axis is a factor: add the factor levels as tick labels 
