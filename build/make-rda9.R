...
lew08 <- lew08 %>% rename(subj = subject, iti = `timed$iti`, 
...
nimmo06 <- nimmo06 %>% rename(postitem.interval = iti)
save(nimmo06, file = "./pkg/data/nimmo06.rda")
...
morin10 <- MorinRecog %>% rename(subj = Subject, age = Age, sex = Sex, trial = Trial,
...
save(morin10, file = "./pkg/data/morin10.rda")
...
frankish89 <- frankish89 %>% rename(cond_num = cond, subj = subject, serpos = spos,
                                    interval = pause)
frankish89 <- frankish89 %>% select(subj, trial, group, condition, cond_num, interval,
                                    serpos, item, resp, acc)

save(frankish89, file="./pkg/data/frankish89.rda")
...
names(exp2b) <- c("subj", "item1", "item2", "item3", "item4", "item5",
                 "item6", "item7", "rt1", "rt2", "rt3", "rt4", "rt5", "rt6"
                 , "rt7", "dist1", "dist2", "dist3", "dist4", "rt_dist1", "rt_dist2"
                 , "rt_dist3", "rt_dist4")
...
farrell09 <- cbind(acc1, rt1$rt)

farrell09$serpos <- as.numeric(substr(farrell09$serpos,5,5))
farrell09 <- farrell09 %>% rename(rt = `rt1$rt`)

farrell09$acc <- as.numeric(farrell09$serpos==farrell09$outpos)

### let's take a look
aggregate(acc ~ serpos, data = farrell09, FUN = mean)
farrell09d$exp <- 2

farrell09d <- farrell09
farrell09a$dist.pc <- NA
farrell09a$dist.rt <- NA
...
farrell09 <- farrell09 %>% select(exp, subj, trial, grouping, dist.pc, dist.rt,
                                  serpos, outpos, acc, rt)
save(farrell09, file = "./pkg/data/farrell09.rda")
