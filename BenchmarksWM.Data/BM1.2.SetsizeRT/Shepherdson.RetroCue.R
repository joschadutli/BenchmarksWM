...
Cue4 <- array(NA,dim=c(4,3,nsubj))
for (ss in 1:4) {
  for (p in 1:3) {
    d <- subset(d1, size==Setsize[ss] & probe==Ptype[p] & CSI=="400")
    aggdat <- aggregate(rt ~ subj_idx, data=d, FUN=mean)
    Cue4[ss,p,] <- aggdat$rt
  }
}
...
### is now about RT's 
lineplot.ci(Setsize, data=Cue4, off=0, xlim=c(1,6), ylim=c(0.5,0.9), 
            pt=ptypes, ptcol=colors[1:3], xlab="Set Size", ylab="Mean RT", main="Cue Delay 0.4 s")
legend(1, 0.5, c("Pos", "New", "Intrus"), pch=ptypes[1:3], pt.bg=colors[1:3], yjust=0)
